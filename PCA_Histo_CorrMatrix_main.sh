...
Rscript PCA_script_v4.R "$DATA_FILE" "$CONDITIONS_FILE" "$PROJECT_NAME" "$OUTPUT_DIR"

# 2. Run Histogram Script
echo "Running Histogram analysis..."
Rscript Histogram_script.R "$DATA_FILE" "$CONDITIONS_FILE" "$PROJECT_NAME" "$OUTPUT_DIR"

# 3. Run Correlation Heatmap Script
echo "Running Correlation Matrix analysis..."
python correlation_matrix_script.py \
  -name "$PROJECT_NAME" \
  -df "$DATA_FILE" \
  -sl "$CONDITIONS_FILE" \
  -op "$OUTPUT_DIR" \
